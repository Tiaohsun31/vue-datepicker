 ...
 *  - 深淺模式：輸出 `data-vdt-mode` 屬性（家族共用同一屬性）；未指定 / auto 時不設，
 *    交由 CSS 的 prefers-color-scheme 跟隨系統。
...
export function useTheme(
    ...
    // 明確指定 light / dark 時設 data-vdt-mode；未指定或 auto 則不設，跟隨系統偏好。
    const themeAttrs = computed<Record<string, string>>(() => {
        const attrs: Record<string, string> = {};
        if (mode.value && mode.value !== 'auto') attrs['data-vdt-mode'] = mode.value;
        return attrs;
    });
